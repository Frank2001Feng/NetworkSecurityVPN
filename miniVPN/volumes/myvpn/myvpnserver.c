...
int main()
{
  ...
  SSL_CTX_use_certificate_file(ctx, "./cert_server/new-server-cert.pem", SSL_FILETYPE_PEM);
  SSL_CTX_use_PrivateKey_file(ctx, "./cert_server/new-server-key.pem", SSL_FILETYPE_PEM);
  ...
  while (1)
  {
    ...
    { // The child process
      ...
      if (login(user, passwd))
      {
        ...
        {
          printf("This IP(%s) has been used!\n", last_ip_buff);
        }
        else
        {
      ...
      SSL_free(ssl);
      close(sock);
      printf("Close socket and return 0.\n");
      return 0;
    }
    else
    {
